...
#ifdef USE_OAUTH2
static gint pop3_getauth_oauth2_send_line_one(Pop3Session *session)
{
	cm_return_val_if_fail(session->user != NULL, -1);
	cm_return_val_if_fail(session->pass != NULL, -1);

	session->state = POP3_GETAUTH_OAUTH2_AFTER_LINE_1;
...
static gint pop3_getauth_oauth2_send_line_two(Pop3Session *session)
{
...
static gint pop3_session_recv_msg(Session *session, const gchar *msg)
{
	...
	case POP3_GREETING:
		...
		if (pop3_session->ac_prefs->use_pop_auth && pop3_session->ac_prefs->pop_auth_type == POPAUTH_OAUTH2)
			val = pop3_getauth_oauth2_send_line_one(pop3_session);
		else
#endif
	...
	case POP3_GETAUTH_USER:
		...
		break;
#ifdef USE_OAUTH2
	case POP3_GETAUTH_OAUTH2_AFTER_LINE_1:
		val = pop3_getauth_oauth2_send_line_two(pop3_session);
		break;
#endif
	case POP3_GETAUTH_PASS:
	case POP3_GETAUTH_APOP:
