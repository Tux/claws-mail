...
my $ppp = qx{perl $ph --compat-version=$cv --quiet perl_plugin.c};
#y $ppp = qx{perl $ph --compat-version=$cv --quiet perl_gtk.c};

$ppp or exit 0;
warn "Devel::PPPort suggests the following change:\n--8<---\n",
    $ppp, "-->9---\n",
    "run 'perl $ph --compat-version=$cv perl_plugin.c' to see why\n";
